"use client";
import { useTheme } from "@/lib/ThemeContext";
import styles from "./Header.module.css";

interface Props {
  address: string | null;
  short: string | null;
  loading: boolean;
  onConnect: () => void;
  onDisconnect: () => void;
}

export default function Header({ address, short, loading, onConnect, onDisconnect }: Props) {
  const { theme, toggleTheme } = useTheme();

  return (
    <header className={styles.header}>
      <div className={styles.left}>
        <div className={styles.logo}>VoxChain</div>
        <div className={styles.tagline}>The People's Ballot</div>
      </div>
      <div className={styles.right}>
        {/* Theme Toggle */}
        <button className={styles.themeToggle} onClick={toggleTheme} aria-label="Toggle theme">
          <div className={`${styles.toggleCircle} ${theme === 'dark' ? styles.toggleDark : ''}`}>
            {theme === 'light' ? (
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="5"/><line x1="12" y1="1" x2="12" y2="3"/><line x1="12" y1="21" x2="12" y2="23"/><line x1="4.22" y1="4.22" x2="5.64" y2="5.64"/><line x1="18.36" y1="18.36" x2="19.78" y2="19.78"/><line x1="1" y1="12" x2="3" y2="12"/><line x1="21" y1="12" x2="23" y2="12"/><line x1="4.22" y1="19.78" x2="5.64" y2="18.36"/><line x1="18.36" y1="4.22" x2="19.78" y2="5.64"/></svg>
            ) : (
              <svg viewBox="0 0 24 24" fill="currentColor"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg>
            )}
          </div>
        </button>

        {address ? (
          <div className={styles.walletGroup}>
            <div className={styles.walletDisplay}>
              <span className={styles.dot} />
              <span className={styles.address}>{short}</span>
            </div>
            <button className={styles.logoutBtn} onClick={onDisconnect} aria-label="Disconnect wallet">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>
            </button>
          </div>
        ) : (
          <button
            className={styles.walletBtn}
            onClick={onConnect}
            disabled={loading}
          >
            {loading ? "Connecting..." : "Connect Wallet"}
          </button>
        )}
      </div>
    </header>
  );
}
